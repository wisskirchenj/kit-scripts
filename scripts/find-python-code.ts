...
const results = await $`cd ${PROJECT_ROOT} ; find * -name *.${FILE_EXT} -exec grep -q ${substring} {} ';' -exec echo "******{}******" ';' -exec grep -${lines} ${substring} {} ';'`;
...
const items = files
    .filter(path => !path.includes("/build/") && !path.includes("/out/"))
    .map(path => ({
        name: path,
